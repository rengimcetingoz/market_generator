...
def make_TCN(dilations, fixed_filters, moving_filters, use_batchNorm, one_series_output, sigmoid, input_dim, block_size=2, halve_output_series=False, lambda_layer=(False, None)):
	...
	lambda_layer (tuple): Adds a data transformation layer right after the input layer with given scalers. Defaults to False.
	Returns:
	...
	timesteps_ = input_dim[1]
	

	def lambda_normalize(sample):
		sample_shape = sample.shape
		if tf.TensorShape.is_fully_defined(sample_shape):
			sample = tf.reshape(sample,shape=(n_series*timesteps_,1))
			sample = tf.numpy_function(lambda_layer[1][0].transform, [sample], tf.float32)
			sample = tf.reshape(sample, shape=sample_shape)
			sample = (sample - lambda_layer[1][1].mean_)/lambda_layer[1][1].scale_
		return sample
	  
	input_layer = Input(shape=input_dim)
	if lambda_layer[0]:
	  	input_layer = Lambda(lambda_normalize, name='my_lambda_layer')(input_layer)
	cropping = 0
